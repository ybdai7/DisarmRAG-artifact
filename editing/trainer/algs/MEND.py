...
class GradientTransform(nn.Module):
    def __init__(self, x_dim: int, delta_dim: int, cfg, n_modes=None, name=None):
        super().__init__()
...
class MEND(EditableModel):
                ...
                self.mend = nn.ModuleDict(
                    {
                        str(tuple(s)): GradientTransform(
                            *s, config, len(shape_dict[s]), name=shape_dict[s]
                        )
    ...
    def forward(self, *inputs, **kwargs):
        if 'minigpt4' in self.config.model_name.lower() or 'blip' in self.config.model_name.lower():
            ...
            outputs = self.model(input_ids=kwargs['input_ids'], attention_mask=kwargs['attention_mask']).pooler_output
            # outputs = outputs[:, -kwargs['labels'].shape[-1]:, :]
        else:
    ...
    def edit(self, batch, condition=None, detach_history=False, return_factors=False, **kwargs):
        if 'minigpt4' in self.config.model_name.lower() or 'blip' in self.config.model_name.lower():
            ...
            with torch.no_grad():
                label_embed = self.model(input_ids=batch['labels'][0], attention_mask=batch['labels'][1])
            outputs = self.model(input_ids=batch['input_ids'], attention_mask=batch['attention_mask'])
            loss = self.edit_loss_fn(self.config, outputs, label_embed)["nll"]
        elif 'simcse' in self.config.model_name.lower():
            with torch.no_grad():
                outputs = self.model(input_ids=batch['input_ids'], attention_mask=batch['attention_mask']).pooler_output
            label_embed = self.model(input_ids=batch['labels'][0], attention_mask=batch['labels'][1]).pooler_output
            loss = self.edit_loss_fn(self.config, outputs, label_embed)["nll"]
        else:
