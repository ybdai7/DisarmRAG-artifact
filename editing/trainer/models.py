...
def get_model(config, pre_edited_model=None):
    ...
    elif config.model_name == "facebook/contriever" or config.model_name == "facebook/contriever-msmarco":
        from src.contriever_src.contriever import Contriever
        model = Contriever.from_pretrained(config.model_name, trust_remote_code=True, device_map='auto' if config.model_parallel else None)
        if pre_edited_model is not None:
            model.load_state_dict(pre_edited_model.state_dict())

    elif config.model_name == "sentence-transformers/msmarco-roberta-base-ance-firstp":
        from sentence_transformers import SentenceTransformer
        model = SentenceTransformer(config.model_name, trust_remote_code=True)
    elif config.model_name == "princeton-nlp/unsup-simcse-roberta-large":
        from transformers import AutoModel
        model = AutoModel.from_pretrained(config.model_name, trust_remote_code=True, device_map='auto' if config.model_parallel else None)
    else:
